...
describe('DeltaSet', () => {
  ...
  describe('constructor', () => {
    it('should create a new set from an iterable', () => {
      const test = new DeltaSet([entry1, entry2]);

      expect(test.size).toEqual(2);
      expect(test.get(entry1.id)).toEqual(entry1);
      expect(test.get(entry2.id)).toEqual(entry2);
    });
  });
